...
def reconocimientoimg():
    ...
    if request.method == 'POST':
        ...
        # Cargar el modelo y realizar la predicción de la imagen
            ...
            if class_name == "." or class_name.lower() == "no reconocido":
                # Si el nombre de la mascota es ".", no registrarla en la base de datos
                flash('No se reconoce la mascota en la imagen.', 'danger')
            else:
                ...
                if len(class_name) >= 3:
                    ...
                    flash('Reconocimiento exitoso y mascota registrada.', 'success')
                else:
                    flash('No se ha llogrado reconocer a la mascota.', 'danger')

            
            registros = RegistroReconocimiento.query.all()
            
            if len(class_name) >= 3:  # Verificar si el nombre de la mascota tiene más de 3 letras
                return render_template('mascotas/reconocimientoimg.html',
                                    class_name=class_name, confidence_score=confidence_score,
                                    nombre_usuario=nombre_usuario, telefono_usuario=telefono_usuario,
                                    registros=registros)
            else:
                return render_template('mascotas/reconocimientoimg.html',
                                    nombre_mascota=class_name, nombre_usuario=nombre_usuario,
                                    telefono_usuario=telefono_usuario, registros=registros)
...
UPLOAD_FOLDER = "C:/Users/user11/Documents/tesis/HuellitasFinal/static/images"
